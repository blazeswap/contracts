...
import '../../core/interfaces/IBlazeSwapManager.sol';
import '../../shared/libraries/Babylonian.sol';
import '../../shared/libraries/FullMath.sol';

import './BlazeSwapLibrary.sol';

// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value
// in terms of the underlying tokens
library BlazeSwapLiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = BlazeSwapLibrary.getReserves(factory, tokenA, tokenB);

        require(reserveA > 0 && reserveB > 0, 'BlazeSwapArbitrageLibrary: ZERO_PAIR_RESERVES');
        ...
        if (aToB) {
            uint256 amountOut = BlazeSwapLibrary.getAmountOut(amountIn, reserveA, reserveB);
            reserveA += amountIn;
            reserveB -= amountOut;
        } else {
            uint256 amountOut = BlazeSwapLibrary.getAmountOut(amountIn, reserveB, reserveA);
            reserveB += amountIn;
            reserveA -= amountOut;
        }
    }
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = BlazeSwapLibrary.getReserves(factory, tokenA, tokenB);
        IBlazeSwapPair pair = IBlazeSwapPair(BlazeSwapLibrary.pairFor(factory, tokenA, tokenB));
        bool feeOn = IBlazeSwapManager(IBlazeSwapFactory(factory).manager()).tradingFeeTo() != address(0);
